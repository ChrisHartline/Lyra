...
| Phase 1 e2e | Partial (Notion pending token) | `venv\Scripts\python scripts/phase1_e2e_demo.py --skip-notion` → ingest arXiv `1802.06002`, 3 cited corpus hits, memory propose/approve gate verified; live Notion digest blocked until `NOTION_TOKEN` + `LYRA_NOTION_DATABASE_ID` in `.env` |
