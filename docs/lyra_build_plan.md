...
| D4 | Gate passed | `venv\Scripts\python -m pytest tests/test_observations.py -q` -> `... [100%]`; full suite: `venv\Scripts\python -m pytest -q` -> `.................. [100%]`; verifies unapproved candidate → zero KG writes, secret-bearing transcript → zero candidates/writes, explicit approval → observation searchable on its entity through the real MCP server |
| D2, D5 | Planned | Phase 2 preview — daily/weekly digest jobs and assistant briefing assembly |
